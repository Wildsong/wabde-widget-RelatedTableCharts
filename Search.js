...
define([
  ...
  return declare([_WidgetBase, Evented], {
    ...
    **/
    ...
    /**
     * This function is used to attach tab event to all the search menu items
     */
    attachTabEventToSearchMenuItems: function () {
      if (this.search) {
        var sourcesMenu = query('.sourcesMenu', this.search.domNode);
        if (sourcesMenu && sourcesMenu.length > 0) {
          if (sourcesMenu[0].children.length > 0) {
            var menuItemsParentContainer = sourcesMenu[0].children[0];
            if (menuItemsParentContainer) {
              var menuItemArr = menuItemsParentContainer.children;
              array.forEach(menuItemArr, lang.hitch(this, function (menuItem, index) {
                this.own(on(menuItem, 'keydown', lang.hitch(this, function (evt) {
                  if (evt.keyCode === keys.TAB) {
                    if (index === (menuItemArr.length - 1)) {
                      Event.stop(evt);
                      this.emit("setFirstFocusNode");
                    }
                  }
                })));
              }));
